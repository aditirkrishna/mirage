...
def test_fit_and_predict():
    ...
    model = MirrorLinearRegression(learning_rate=0.2, n_iters=500, lam=0.05, tol=1e-7, verbose=True)
    model.fit(X, y)
    ...
    assert mse < 0.05  # Should be a low error for synthetic data

if __name__ == "__main__":
    test_fit_and_predict()
    print("Core model test passed.")
